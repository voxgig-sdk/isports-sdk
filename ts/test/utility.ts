...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.ISPORTS_TEST_LIVE ||
    'TRUE' === process.env.ISPORTS_TEST_OVERRIDE
  ) {
  ...
  m.ISPORTS_TEST_EXPLAIN = process.env.ISPORTS_TEST_EXPLAIN || m.ISPORTS_TEST_EXPLAIN
