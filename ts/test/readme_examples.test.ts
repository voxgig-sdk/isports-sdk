...
const TEST_SEED = {"entity":{"football":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+IsportsSDK\b/.test(code) ||
    /\bIsportsSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IsportsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IsportsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
