...
class FtCtrl extends IsportsControl
{
...
class FtClient
{
    ...
    private IsportsUtility $utility;

    public function __construct(IsportsUtility $utility, array $options)
    {
    ...
    public function get_utility(): IsportsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IsportsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IsportsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IsportsConfig::make_feature($name);
    ...
    private static function buildUrl(IsportsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IsportsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IsportsResponse($fetched);
        }
    ...
    private function fail(IsportsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IsportsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
