...
// Isports SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        IsportsSDK $client,
        IsportsUtility $utility
    ): IsportsContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new IsportsContext($ctxmap, null);
    ...
    private static function fixctx(IsportsContext $ctx, IsportsSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?IsportsError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IsportsError($code, $msg);
    }
    ...
    {
        $client = IsportsSDK::test(null, null);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new IsportsBaseFeature();
    ...
    {
        $hook_client = IsportsSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends IsportsBaseFeature {
            public $hook_fn = null;
            public function TestHook(IsportsContext $ctx): void
            {
    ...
    {
        $init_client = IsportsSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends IsportsBaseFeature {
            public $init_fn = null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new IsportsSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = IsportsSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = IsportsSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof IsportsError) ? $result->err->msg : (string)$result->err;
