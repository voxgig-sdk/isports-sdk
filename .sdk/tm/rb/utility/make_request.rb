...
module IsportsUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = IsportsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IsportsResponse.new(fetched)
    else
