// Command Isports-cli is a CLI + REPL for the Isports SDK,
...
const prompt = "isports"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "football"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ISPORTS_APIKEY (api key), ISPORTS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ISPORTS_APIKEY carries the API key and
	// ISPORTS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ISPORTS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ISPORTS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIsportsSDK(opts)
