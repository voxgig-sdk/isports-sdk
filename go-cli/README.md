...
boru-driven command-line client **and** interactive REPL for the Isports
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/isports-cli)
make build
...
./isports-cli load 1 football            # {id:1} shorthand
./isports-cli load '{id:1}' football       # explicit match map

# 5. Override the API base URL for a single call
ISPORTS_BASE=https://api.example.com ./isports-cli load 1 football

# 6. No arguments -> interactive REPL
./isports-cli
isports> load 1 football
isports> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/isports-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/isports-cli load 1 football
   ```
...
```sh
./isports-cli load 1 football          # scalar shorthand for {id:1}
./isports-cli load '{id:1}' football     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `isports>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `football`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/isports-cli`. |
...
(`./isports-cli <expr>`) and interactively (the REPL), and expressions compose the
