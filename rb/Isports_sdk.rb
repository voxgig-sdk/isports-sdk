...
require_relative 'Isports_types'
...
class IsportsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = IsportsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = IsportsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, IsportsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = IsportsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = IsportsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = IsportsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = IsportsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = IsportsError.new(
        "graphql_error", "IsportsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Football.list / client.Football.load({ "id" => ... })
  def Football(data = nil)
    require_relative 'entity/football_entity'
    FootballEntity.new(self, data)
  end
