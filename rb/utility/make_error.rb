...
module IsportsUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = IsportsContext.new({}, nil)
    end
    op = ctx.op || IsportsOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || IsportsResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(IsportsError) ? err.msg : err.to_s
    ...
    sdk_err = IsportsError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(IsportsError)
