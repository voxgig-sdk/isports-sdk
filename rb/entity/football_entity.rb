...
class FootballEntity
  ...
  def make
    opts = @_entopts.dup
    FootballEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IsportsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Football, Hash] the current Football data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IsportsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Football fields)
  def match_get
  ...
  #
  # @param reqmatch [FootballLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Football.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Football, Hash] the loaded Football; raises IsportsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IsportsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
