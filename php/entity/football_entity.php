...
class FootballEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new FootballEntity($this->_client, $opts);
    }

    /**
     * @param Football|array $args Football data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IsportsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Football|array The current Football data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Football fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IsportsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Football fields).
     */
    ...
    /**
     * Load a single Football.
     *
     * @param FootballLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed FootballLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Football|array The loaded Football as an assoc-array at the
     *   SDK boundary; throws IsportsError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IsportsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
