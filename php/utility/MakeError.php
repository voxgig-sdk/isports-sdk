...
class IsportsMakeError
{
    public static function call(?IsportsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IsportsContext([], null);
        }
        $op = $ctx->op ?? new IsportsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IsportsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IsportsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IsportsError('', $msg, $ctx);
        ...
        if ($err instanceof IsportsError) {
