...
class FootballDirectTest extends TestCase
{
    public function test_direct_load_football(): void
    {
        $setup = football_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-football", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function football_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new IsportsSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
