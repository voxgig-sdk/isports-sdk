...
[MCP](https://modelcontextprotocol.io) server exposing the Isports SDK as
two agent tools — `isports_list` and `isports_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/isports-mcp)
make build

# 2. Provide credentials via the environment
export ISPORTS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user isports \
  -- /absolute/path/to/isports-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./isports-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// isports_list: first page of records
{ "entity": "football" }
{ "entity": "football", "query": { } }

// isports_load: one record by id
{ "entity": "football", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/isports-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user isports \
     -- "$PWD"/dist/*/isports-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `isports_list` and `isports_load` tools now appear
   in new sessions. Ask the agent to *"list football using isports"*
   and it calls `isports_list` with `{"entity":"football"}`.
...
```sh
export ISPORTS_APIKEY=sk_live_xxx            # API key
export ISPORTS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `isports_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `isports_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ISPORTS_APIKEY` | API key sent with every request. |
| `ISPORTS_BASE` | Optional override of the API base URL. |
...
```sh
./isports-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"isports_load","arguments":{"entity":"football","query":{"id":1}}}}'
```
