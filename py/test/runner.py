...
class IsportsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IsportsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IsportsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IsportsTestRunner.getenv("ISPORTS_TEST_LIVE")
        override = IsportsTestRunner.getenv("ISPORTS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IsportsTestRunner.getenv(key)
        ...
        explain = IsportsTestRunner.getenv("ISPORTS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ISPORTS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IsportsTestRunner._test_control is not None:
            return IsportsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IsportsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IsportsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IsportsTestRunner.load_test_control()
...
def load_env_local():
    IsportsTestRunner.load_env_local()


def env_override(m):
    return IsportsTestRunner.env_override(m)


def entity_data(v):
    return IsportsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IsportsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IsportsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IsportsTestRunner.load_test_control()


def live_delay_ms():
    return IsportsTestRunner.live_delay_ms()
