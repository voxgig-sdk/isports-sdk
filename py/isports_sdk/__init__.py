...
from isports_sdk.utility import register

# Load features
from isports_sdk.feature.base_feature import IsportsBaseFeature
from isports_sdk.features import _make_feature
...
class IsportsSDK:
    ...
    def __init__(self, options=None):
        ...
        utility = IsportsUtility()
        self._utility = utility

        from isports_sdk.config import make_config
        config = make_config()
    ...
    def get_utility(self):
        return IsportsUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("IsportsSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def Football(self, data=None) -> "FootballEntity":
        """Entity factory: client.Football().list() / client.Football().load({"id": ...})."""
        from isports_sdk.entity.football_entity import FootballEntity
        return FootballEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "IsportsSDK":
        if sdkopts is None:
            sdkopts = {}
