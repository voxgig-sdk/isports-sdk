...
local FootballEntity = {}
FootballEntity.__index = FootballEntity


function FootballEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FootballEntity)
  self._name = "football"
  self._client = client
...
function FootballEntity:get_name()
  return self._name
end


function FootballEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FootballEntity.new(self._client, opts)
end
...
function FootballEntity:mark_deleted()
  self._deleted = true
end


function FootballEntity:deleted()
  return true == self._deleted
end


function FootballEntity:data_set(args)
  if args ~= nil then
...
function FootballEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FootballEntity:match_set(args)
  if args ~= nil then
...
function FootballEntity:match_get()
...
function FootballEntity:stream(action, args, callopts)
...
---@param reqmatch FootballLoadMatch
---@param ctrl? table
---@return Football
---@return string? err
function FootballEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function FootballEntity:_run_op(ctx, post_done)
  local utility = self._utility
